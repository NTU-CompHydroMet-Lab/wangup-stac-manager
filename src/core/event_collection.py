...
# Project-specific product display titles live in deployment repos, not the engine.
# Engine falls back to the alias string itself when no override is provided.
...
def select_event_assets(alias: str, assets: dict) -> list[tuple[str, dict]]:
    ...
    TODO: alias-specific selection logic below (e.g. iot_timeseries → parquet)
    is project-specific. Deployment repos should be able to register custom
    selectors. Until that hook exists, the iot_timeseries branch is dead code
    in the engine and only fires when a deployment configures that alias.
    """
